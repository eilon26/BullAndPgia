...
/* *************************************************************************** */
	...
	for (uint i=0; i<100; ++i) {
		...
		if( len < 7 ) {
		testcase.CHECK_EQUAL(play(randy, smarty, len , 100) <= 10 , true);  // smarty should always win in at most 10 turns!
		}
		else {
		testcase.CHECK_EQUAL(play(randy, smarty, len , 100) <= 50 , true);  // smarty should always win in at most 50 turns!
		}
		...
		cout << len << " Length, Guessed In " << elaspedTimeMs << " MilliSeconds. " << endl;
